...
\begin{document}
	...
	%Also to my girlfriend Jen Marshall, for driving me on and proof reading this report.
	\end{center}
	\end{abstract}
...
%	Word Count: \bashStdout.
	...
	\chapter{Background Research}\label{chap:background}
			...
			When using two cameras, as those that are to be provided by the LMC, \citeasnoun[p. 415]{definition:cv} define four steps that are needed to achieve successful correspondence between the images produced by each.
			...
			As this is a fundamental element to this project, each stage of calibration will be covered in a little more detail.
				...
				There is a lot of information available on how distortion happens so only the basics will be covered. %, however some reading suggestions are (READING SUGGESTIONS HERE).
				...
				The following description and figures of this model is taken from \citeasnoun[p. 371-373]{definition:cv}, If more depth is needed the reader should refer to the cited material.
				...
				For mathematical simplification \citeasnoun{definition:cv} states that the negative value can be omitted leaving $x=f\dfrac{X}{Z}$.
				...
				\subsection{Image Rectification}
					...
					For more in depth information on image rectification please read \citeasnoun[p. 419-421]{definition:cv}.
				\begin{figure}
				\centering
				...
				\subsection{Stereo Correspondence}
					...
					Both \citeasnoun{journal:matchers} and \citeasnoun{journal:matchers2} carry out a review on these methods.
			...
			\section{Classification}
				...
				\subsection{Extracting Image Features}
					...
					\end{quote}
					Where as, \citeasnoun[p. 179]{book:modern} describes corners as being a local area where the orientation of the image gradient changes sharply; a corner in the more literal sense of the word.
					\citeasnoun{book:modern} says a feature is a more specific term of something that can be described as an image point.
				...
				Attempts have been made at classifying point clouds. A piece of software called CANUPO has been developed based on methods outlined by \citeasnoun{journal:3dclass}.
	...
	\section{Iteration 1}\label{sec:p1}
		...
		\lstinputlisting[caption=Convert \code{Mat} type to \code{javafx.scene.image.Image} type, label=lst:matToImage]{matToImage.java}	
				
		
		\clearpage		
		\lstinputlisting[caption=Crop \code{Mat} Image, label=lst:crop]{cropImage.java}		
		\begin{figure}
			\centering
		...
		StereoSGBM stands for semi-global block matching, an algorithm that was researched by \citeasnoun{journal:stereoSGBM}.
		StereoVar stands for stereo variational methods, which is an algorithm produced by \citeasnoun{journal:stereoVar}. 
		...
		\lstinputlisting[caption=Initialisation of Distortion Data, label=lst:stereoSGBM]{computeDisparity.java}
	...
	\section{Iteration 2}\label{sec:p2}
		...
		\subsubsection{Stage 3 - Distortion Removal Optimisation}
			...
			The method used here was converted from C++ code into Java, originally written by \citeasnoun{web:bilin}.
			...
			\clearpage
			\lstinputlisting[caption=Initialisation of Distortion Data, label=lst:code_10]{initialiseDistortionMats.java}
			\clearpage
		\subsection{Test}
			\subsubsection{Performance Tests}
			\input{performance_test_1}
	...
	\section{Iteration 3}\label{sec:p3}
		\subsection{Design}
			\subsubsection{Class Diagram}
				...
				A decision was made to implement this following the factory design pattern as defined in \citeasnoun{book:head}.
		...
		\end{figure}	
			...
			A method was outlined on t\citeasnoun{url:distortion} might prove useful.
			...
			\citeasnoun{blog:leap} does provide some values obtained from calibration of the LMC.
		...
		\subsubsection{Stage 2 - Three Dimensional Reprojection}
			...
			\lstinputlisting[caption=Writing Generated Point Cloud to a File, label=lst:code_9]{savePointCloud.java}
	...
	\section{Reflection on Research Approach}
		...
		The research process used was guided by the methods outlined by \citeasnoun{book:dawson}.
	...
	\chapter{CRCs for Iteration 2} \label{app:crc_it_1}
			...
			\input{crc_leapCVStereoUtils}
			\input{crc_leapCVImageUtils}
			\input{crc_LeapCVStereoCalibrator}
	\chapter{CRCs for Iteration 3} \label{app:crc_it_2}
			\input{crc_LeapCVCalibrationUtils}
			\input{crc_LeapCVStereoMatcherFactory}
			\input{crc_LeapCVStereoMatcher}
			\input{crc_LeapCVStereoVar}
			\input{crc_LeapCVStereoSGBM}
			\input{crc_LeapCVStereoBM}
			\input{crc_MatcherType}
	...
	\chapter{Implementation Code Snippets}
			...
			\clearpage
			\lstinputlisting[caption=Example JUnit Unit Test Case, label=lst:test_case]{testCase.java}
			\clearpage
			\lstinputlisting[caption=Failed Calibration Attempt, label=lst:calibrate]{calibrateCameras.java}
			\clearpage
			\lstinputlisting[caption=Feature Detection, label=lst:feature1]{detectObjects.java}
			\clearpage
			\lstinputlisting[caption=Outlier Removal, label=lst:outliers]{removeOutliers.java}
			\clearpage
			\lstinputlisting[caption=Removal of Distortion from an Image using Bilinear Interpolation. Converted from C++ to Java, label=lst:bilin]{bilin.java}
	\end{appendices}
